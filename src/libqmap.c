		...
		const void * const value, unsigned pn)
{
	...
	if (key) {
		...
		if (qmap->flags & QMAP_AINDEX) {
			...
			n = pn == QMAP_MISS ? idm_new(&qmap->idm) : pn;
	} else {
			n = pn == QMAP_MISS ? idm_new(&qmap->idm) : pn;
		id = n;
		rkey = &n;
